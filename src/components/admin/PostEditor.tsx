...
export default function PostEditor({ post }: { post?: Post }) {
  ...
  const youtubeCommand: ICommand = {
    ...
    icon: <span className="material-symbols-outlined" style={{ fontSize: 16 }}>smart_display</span>,
  ...
  return (
    <div style={{ maxWidth: mode === 'split' ? 1400 : 900 }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 4 }}>
          {([['edit', 'edit', 'Edit'], ['split', 'vertical_split', 'Split'], ['blog', 'visibility', 'Blog']] as const).map(([m, icon, label]) => (
            <button key={m} onClick={() => setMode(m)} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 12px', borderRadius: 6, border: 'none', cursor: 'pointer', fontSize: 12, background: mode === m ? '#3e3c8f' : '#1e1e2e', color: mode === m ? '#c0c1ff' : '#64748b', fontWeight: mode === m ? 700 : 400 }}>
