...
const AMBIENT = [
  { x: 0.08, y: 0.12, r: 1.0, ph: 0.0 },
  { x: 0.92, y: 0.18, r: 1.2, ph: 0.8 },
  { x: 0.15, y: 0.65, r: 0.8, ph: 1.5 },
  { x: 0.85, y: 0.58, r: 1.1, ph: 2.3 },
  { x: 0.25, y: 0.08, r: 0.9, ph: 0.4 },
  { x: 0.72, y: 0.06, r: 0.7, ph: 1.2 },
  { x: 0.05, y: 0.42, r: 1.0, ph: 3.1 },
  { x: 0.95, y: 0.36, r: 0.8, ph: 2.7 },
  { x: 0.2, y: 0.88, r: 0.9, ph: 0.9 },
  { x: 0.8, y: 0.85, r: 1.1, ph: 1.8 },
  { x: 0.45, y: 0.04, r: 0.7, ph: 2.1 },
  { x: 0.55, y: 0.96, r: 0.8, ph: 0.3 },
  { x: 0.12, y: 0.35, r: 0.5, ph: 0.6 },
  { x: 0.88, y: 0.3, r: 0.6, ph: 1.4 },
  { x: 0.35, y: 0.92, r: 0.8, ph: 2.4 },
  { x: 0.65, y: 0.9, r: 0.7, ph: 0.2 },
];

const GROUPS = [
  { starStart: 0.15, lineStart: 0.3, starIndices: [0, 1, 2], lineIndices: [0, 1] },
...
export function TwentyOnePilotsEgg({ onClose }: { onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    // Theme-aware star/line color base
    const starRgb = isDark ? '192,193,255' : '64,66,200';
    const starCorRgb = isDark ? '225,226,255' : '40,42,180';
    ...
    const draw = (now: number) => {
      ...
      AMBIENT.forEach((s) => {
        ...
        ctx.fillStyle = `rgba(${starRgb},${ambientIn * 0.55 * twinkle})`;
        ctx.fill();
      });
      ...
      GROUPS.forEach((group) => {
        group.starIndices.forEach((si, gi) => {
          const p = easeOut(
            remap(t, group.starStart + gi * 0.055, group.starStart + gi * 0.055 + 0.1)
          );
          if (p <= 0) return;
          ...
          grd.addColorStop(0, `rgba(${starRgb},${0.55 * p})`);
          ...
          ctx.fillStyle = `rgba(${starCorRgb},${p})`;
          ctx.fill();
        });
        ...
        group.lineIndices.forEach((li, gi) => {
          ...
          const p = easeOut(
            remap(t, group.lineStart + gi * 0.09, group.lineStart + gi * 0.09 + 0.18)
          );
          if (p <= 0) return;

          const sx = STARS[from].x * W,
            sy = STARS[from].y * H;
          const ex = STARS[to].x * W,
            ey = STARS[to].y * H;

          ctx.beginPath();
          ctx.moveTo(sx, sy);
          ctx.lineTo(sx + (ex - sx) * p, sy + (ey - sy) * p);
          ctx.strokeStyle = `rgba(${starRgb},${0.65 * Math.min(p * 2.5, 1)})`;
          ctx.lineWidth = 1.5;
          ctx.stroke();
        });
      });
  ...
  return (
    <motion.div
      ...
      style={{ backgroundColor: 'color-mix(in srgb, var(--ds-bg) 95%, transparent)', backdropFilter: 'blur(3px)' }}
      onClick={onClose}
    >
      ...
      <motion.p
        ...
        style={{ color: 'var(--ds-on-surface)', fontFamily: 'var(--font-inter), sans-serif' }}
      >
